    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(actionType != null && actionType.equals("delete")) {
                ...
                {
                    String check = userList.get(x).getEmailAddress();
                    if(emailToEdit.equals(check))
                    {
                        emailToEdit = userList.get(x).getEmailAddress();
        ...
        if(action.equals("addUser"))
            {
                ...
                {
                    if(roleList.get(x).getName().equals(roleName))
                    {
                        role = roleList.get(x);
                    }
                }
            ...
            try {
                ...
                try {
                    ...
                    for(int x = 0; x < userList.size(); x++)
                    {
                        ...
                        {
                            userService.update(userList.get(x).getEmailAddress(),userList.get(x).getIsActive(),firstName,lastName,userList.get(x).getPassword(),role);
                        }
                    }
